...
def ensure_schema_migration_table_exists(database):
    ...
    if not schema_migration_table_exists(database):
        ...
        database.execute(f"CREATE TABLE {TABLE_NAME_SQLITE_SCHEMA_MIGRATE} (schema TEXT, applied_at DATETIME);")
    else:
...
def update_schema_migration_table(database, schema):
    ...
    if first_item is None:
        database.execute(f"INSERT INTO {TABLE_NAME_SQLITE_SCHEMA_MIGRATE} (schema, applied_at) VALUES (?, ?);",
                         (schema, datetime.datetime.now()))
        database.commit()
    else:

        log.debug("Schema migration already exists... updating")
        database.execute(f"UPDATE {TABLE_NAME_SQLITE_SCHEMA_MIGRATE} SET schema = ?, applied_at = ? WHERE rowid = ?;",
